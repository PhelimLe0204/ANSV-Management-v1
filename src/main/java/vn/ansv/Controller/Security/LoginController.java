...
public class LoginController {
	...
	public @ResponseBody String compareRole(HttpServletRequest request) {
		...
		for (int i = 1; i <= Integer.parseInt(size_role); i++) {
			...
			// Vòng lặp kiểm tra role lớn nhất user có, so sánh với role được cấp trên
			// database => Nếu khác, thực hiện update role mới trên database
			for (int j = 0; j < 5; j++) {
	...
	public String loginSuccess(HttpServletRequest request, HttpSession session, Model model,
		...
		if (current_week == 1) {
			// Nếu tuần hiện tại lấy được là tuần đầu tiên của năm nay (tuần 1) => "Tuần
			// trước" sẽ là tuần cuối cùng của năm trước
		...
		if (authentication != null) {
			...
			// int role_quantity = userDetails.getAuthorities().size(); // Lấy ra số lương
