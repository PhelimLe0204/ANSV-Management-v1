...
public class AccountManagerController extends AccountManagerBaseController {
	...
	public ModelAndView AmHome(@PathVariable int week, @PathVariable int year,
			@RequestParam(required = false) String message, HttpSession session, Model model) {
	...
	public ModelAndView AmCreateProject(@PathVariable int week, @PathVariable int year, Model model,
			HttpSession session) {
		String pic_id = (String) session.getAttribute("user_id");
		InitAM(week, year, pic_id);
	...
	public String doInsertProject(@PathVariable int week, @PathVariable int year,
			@ModelAttribute("Project") Project project, Model model, HttpSession session) {
		String pic_id = (String) session.getAttribute("user_id");
	...
	public ModelAndView AmUpdateProject(@PathVariable int week, @PathVariable int year, @PathVariable int id,
	...
	public String doUpdateProject(@ModelAttribute("Project") Project project, @PathVariable int week,
			@PathVariable int year, @PathVariable int id, Model model, HttpSession session) {
		...
		if (project.getWeek() == current_week) {
			...
			return "redirect:/AM/dashboard/" + week_link + "_" + year; // Sau khi thực hiện insert, điều hướng về thời
																		// điểm hiện tại
		}
	...
	public ModelAndView nextStepProject(@ModelAttribute("Project") Project project, @PathVariable int week,
		...
		_mvShare.addObject("project_old", _projectService.getFormDeployment(id)); // Lấy ra thông tin dự án trước khi
	...
	public String doDeploymentTest(@ModelAttribute("Project") Project project, Model model, HttpSession session) {
		...
		_projectService.saveDeploymentTest(project); // Lưu dự án Triển khai (cột note sẽ chứa "id" của dự án giai đoạn
														// trước)
	...
	public String doDeployment(@ModelAttribute("Project") Project project, @PathVariable int week,
			@PathVariable int year, @PathVariable int id, Model model, HttpSession session) {
		String am_id = (String) session.getAttribute("user_id");
		...
		if (week == current_week) {
			...
			_projectService.saveDeployment(project, id); // Lưu dự án Triển khai (cột note sẽ chứa "id" của dự án giai
															// đoạn trước)
			_picService.save(project.getId(), am_id); // Insert Acount Manager (người tạo dự án)
			_picService.save(project.getId(), project.getNote()); // Insert PIC được uỷ quyền thực hiện
		}
	...
	public String doInsertCustomer(@PathVariable int week, @PathVariable int year,
			@ModelAttribute("customer_form") Customer customer, Model model, HttpSession session) {
		String pic_id = (String) session.getAttribute("user_id");
	...
	public String AmUpdateCustomer(@ModelAttribute("customer_form") Customer customer, @PathVariable int id,
			HttpSession session, Model model) {
		Date now = new Date();
		int current_week = getWeekOfYear(now); // Gọi hàm lấy số tuần => Lấy số tuần hiện tại
		int current_year = Calendar.getInstance().get(Calendar.YEAR); // Get the curent year
